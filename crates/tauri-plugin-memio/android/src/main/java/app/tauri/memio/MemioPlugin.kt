...
/**
 * MemioPlugin - Provides zero-copy shared memory access for Tauri Android apps.
 * 
 * ## Architecture: C++ Data Plane + JS Controller
 * 
 * This plugin uses multiple approaches for optimal performance:
 * 
 * 1. **JavaScriptInterface** - For commands and metadata (small payloads)
 * 2. **memio:// protocol** - For downloads (zero Base64, 15x faster!)
 * 3. **Native file upload** - For uploads via content:// URI (zero Base64!)
 * 
 * ## Performance:
 * - Download: Base64 ~740ms → memio:// ~50ms (15x faster!)
 * - Upload: Base64 ~300ms → Native URI ~20ms (15x faster!)
 * 
 * The plugin wraps Tauri's WebView clients without breaking functionality.
 */
@TauriPlugin
class MemioPlugin(private val activity: Activity) : Plugin(activity) {
     ...
     * 
     * Expected performance: ~10-30ms for 5MB (vs ~300ms with Base64)
     */
    @Command
