//! Windows shared memory integration for Tauri.
//!
//! This module provides Tauri commands for zero-copy data transfer
...
//! 3. JS writes data directly to the ArrayBuffer (zero-copy)
...
//! 4. JS reads data from the ArrayBuffer (zero-copy)
...
/// Creates a new Windows shared memory region.
#[command]
...
/// Lists all Windows shared memory regions.
#[command]
...
/// Prepare a SharedBuffer for zero-copy upload.
...
pub async fn prepare_upload_buffer<R: Runtime>(
    ...
    use webview2_com::Microsoft::Web::WebView2::Win32::{
        ICoreWebView2_2, ICoreWebView2Environment12, ICoreWebView2_17, 
        COREWEBVIEW2_SHARED_BUFFER_ACCESS_READ_WRITE
    };
    use windows::core::PCWSTR;
    
    tracing::info!("[MemioWindows] prepare_upload_buffer: '{}' ({} bytes)", name, size);
...
/// Commit an upload - read data from SharedBuffer and write to shared memory.
#[command]
pub async fn commit_upload_buffer(
    name: String,
    version: u64,
    length: usize,
) -> Result<(), String> {
...
/// Send data via zero-copy SharedBuffer (back→front).
/// Creates a WebView2 SharedBuffer, copies data from shared memory, and posts it to JS.
#[command]
