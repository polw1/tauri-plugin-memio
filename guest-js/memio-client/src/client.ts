...
export class MemioClient {
   ...
   * This allows the client to detect memio region that becomes available after construction.
   */
  private get provider(): SharedStateProvider {
    // Always recreate provider if current one is unknown but memio region is now available
  ...
  /**
   * Returns true if memio region is available.
   */
  ...
  /**
   * Reads a named memio buffer as raw bytes.
   */
