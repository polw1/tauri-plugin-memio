 ...
 * 
 * This module provides zero-copy data transfer between JavaScript and Rust
 ...
 * These are SharedBuffers with ReadWrite access for zero-copy uploads.
 */
const pendingUploadBuffers = new Map<string, ArrayBuffer>();

/**
 * Pending download buffers received from Rust.
 * These are SharedBuffers with ReadOnly access for zero-copy downloads.
 */
...
/**
 * Upload data using WebView2 SharedBuffer (zero-copy).
 * 
 * Flow:
...
export async function uploadViaSharedBuffer(
  ...
  try {
    ...
    // Step 3: Write data directly to the SharedBuffer (ZERO COPY!)
...
/**
 * Download data using WebView2 SharedBuffer (zero-copy).
 * 
 * Flow:
