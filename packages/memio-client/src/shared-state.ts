...
/**
 * Reads data from shared memory buffer (zero-copy on Windows).
 * - Linux: Uses WebKit extension globals
 * - Android: Uses memio:// protocol ONLY (ZERO BASE64! NO FALLBACK!)
 * - Windows: Uses WebView2 SharedBuffer API (zero-copy!)
 * 
...
export async function readMemioSharedBuffer(
  ...
  // Windows: Use SharedBuffer for zero-copy download
...
export async function writeMemioSharedBuffer(
  ...
  // Windows: ONLY SharedBuffer (zero-copy) - NO FALLBACKS for testing
